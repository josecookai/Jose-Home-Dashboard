...
def fetch_strava_today(today: str) -> list[dict]:
    ...
    return _supabase_get(
        "strava_activities",
        {"select": "name,activity_type,distance_km,duration_min,date", "date": f"gte.{today}"},
    )
...
def build_html_email(
    ...
    for m in sorted(module_statuses, key=lambda x: x.get("module_name", "")):
        name = html.escape(str(m.get("module_name", "—")))
        last_run = html.escape(str(m.get("last_run_at", "—")))
        status = m.get("last_status")
        message = m.get("last_message", "")
        # Truncate long messages to keep table readable
        short_msg = html.escape((message[:120] + "…") if len(message) > 120 else message)
        status_rows_html += f"""
        <tr>
    ...
    if strava_rows:
        ...
        for act in strava_rows:
            ...
            <tr>
                ...
                <td>{_duration_str(act.get('duration_min'))}</td>
            </tr>
            """
        strava_html = f"""
        <div class="section">
...
<div class="wrapper">
    ...
    {strava_html}
    {reports_html}
    <div class="footer">Generated by Jose Home Dashboard · {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M")} UTC</div>
</div>
</body>
</html>"""
